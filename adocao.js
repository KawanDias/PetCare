...
pets = pets.map((pet) => {
  ...
  const emoji = speciesEmojiMap[especie] || pet.emoji || '';
...
function renderPets() {
    ...
    const availablePets = pets
        ...
        .filter(({ pet }) => !pet.adotadoPor);

    if (availablePets.length === 0) {
        renderCarousel();
        grid.innerHTML = '<p style="text-align: center; grid-column: 1/-1; padding: 40px;">Nenhum pet disponível no momento. Confira os pets já adotados na aba "Adotados".</p>';
        return;
    }

    grid.innerHTML = availablePets.map(({ pet, index }) => {
        ...
        const adoptButton = `<button class="primary" onclick="event.stopPropagation(); adoptPet(${index})">❤️ Adotar</button>`;
...
function setupFormListener() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const pet = {
            ...
            emoji: speciesEmojiMap[selectedSpecies] || '',
            criador: currentUser,
        ...
        if (editingId !== null) {
            const originalPet = pets[editingId];
            if (originalPet.criador !== currentUser) {
                showAlert('Você só pode editar seus próprios pets!', 'error');
                return;
            }
            if (originalPet.adotadoPor) {
                showAlert('Pets adotados não podem mais ser editados.', 'error');
                return;
            }

            // Editar pet existente (manter criador original)
            pet.criador = originalPet.criador;
            pet.dataCriacao = originalPet.dataCriacao;
            pet.adotadoPor = originalPet.adotadoPor;
            pet.adotadoPorId = originalPet.adotadoPorId;
            pet.dataAdocao = originalPet.dataAdocao;
            pets[editingId] = pet;
            editingId = null;
